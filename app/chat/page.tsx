...
const agents = [
  ["Jackie", "Organizza il caos", "Summary e richiesta"],
  ["Milo", "Trova priorità e task", "Next actions"],
  ["Nora", "Prepara risposte", "Reply draft"],
  ["Dex", "Classifica e collega", "Tag e status"],
];
...
const transcript = [
  {
    ...
    text: "ciao, io e mio fratello dobbiamo fare una cosa per il negozio... sì voglio dire un sito, ma magari anche la gestione social? non lo so ancora bene. comunque ci serviva entro fine mese tipo. ah, e non abbiamo budget enorme, max 800€ forse. dimmi tu",
  },
  {
    side: "ai",
    label: "Jackie",
    text: "Ho ripulito il messaggio: il cliente vuole un sito per il negozio, forse anche gestione social. Ha una scadenza entro fine mese e un budget massimo indicativo di 800€.",
  },
  {
    side: "ai",
    label: "Milo",
    text: "Task: chiarire se il sito è vetrina o e-commerce, capire se i social sono setup o gestione mensile, preparare un preventivo entro 24 ore.",
  },
  {
    side: "ai",
    label: "Nora",
    text: "Risposta pronta: Ciao! Certo, possiamo parlarne. Per capire meglio: il sito deve essere solo vetrina o deve anche vendere online? E per i social pensavate a una gestione mensile o solo a un setup iniziale?",
  },
];
    ...
    <main className="flow-lime-glow min-h-screen overflow-hidden bg-[var(--fc-bg)] text-[var(--fc-text)]" id="main-content" tabIndex={-1}>
      ...
      <div className="relative z-10 mx-auto grid min-h-screen max-w-[1500px] gap-4 px-4 py-4 lg:grid-cols-[280px_minmax(0,1fr)_340px]">
        <aside className="fc-panel hidden p-4 lg:block">
          ...
          <p className="fc-label">Demo preview</p>
        ...
        <section className="fc-panel grid min-h-0 grid-rows-[auto_1fr_auto] overflow-hidden">
          <header className="flex items-center justify-between gap-4 border-b border-white/[0.06] px-5 py-4 max-sm:flex-col max-sm:items-start">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="fc-label">Demo</p>
                <h2 className="text-2xl font-extrabold tracking-[-0.05em]">Vedi FlowCrew in azione</h2>
              </div>
            </div>
            <Link className="fc-button fc-button-primary" href="/trial">Prova un lead</Link>
          </header>
          ...
          <footer className="border-t border-white/[0.06] p-4">
            <div className="mx-auto flex max-w-4xl flex-col gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.025] p-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm leading-6 text-[var(--fc-text-muted)]">Demo statica. Per analizzare un messaggio reale usa il trial gratuito.</p>
              <Link className="fc-button fc-button-primary shrink-0" href="/trial">Prova gratis</Link>
            </div>
          </footer>
        </section>

        <aside className="fc-panel p-4 lg:overflow-auto">
          <h2 className="text-2xl font-extrabold tracking-[-0.05em]">Output generati</h2>
          <div className="mt-5 grid gap-3">
            {[
              [FileText, "Summary", "Richiesta sito + social, budget 800€, scadenza stretta."],
              [Clock3, "Priorità", "Lead caldo da qualificare subito."],
              [MessageSquareText, "Reply", "Domande chiare per definire scope e preventivo."],
              [Tags, "Tags", "lead-caldo, sito-web, budget-limitato"],
            ].map(([Icon, title, body]) => {
          ...
          <div className="mt-5 rounded-2xl border border-[rgba(139,255,197,0.22)] bg-[rgba(139,255,197,0.08)] p-4">
            <div className="flex items-center gap-2 text-[var(--fc-mint)]">
              ...
              <p className="font-bold">Approval-first</p>
            </div>
